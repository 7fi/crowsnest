    ...
    <div>
      ...
      <div className='fullContentBox'>
        <div className='flexRowContainer'>
                ...
                {teams
                  ...
                  .map((team, index) => (
                    <tr className='clickable' onClick={() => nav(`/rankings/team/${team.name}`)}>
                      ...
                      <td>{team.name}</td>
                      <td className='tableColFit'>{team.topRating.toFixed(0)}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
                ...
                {teams
                  ...
                  .map((team, index) => (
                    <tr className='clickable' onClick={() => nav(`/rankings/team/${team.name}`)}>
                      ...
                      <td>{team.name}</td>
                      <td className='tableColFit'>{team.topWomenRating.toFixed(0)}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
          ...
          {/* <div className='flexGrowChild contentBox'>
            <h2 className='clickable' onClick={() => nav(`/rankings/skipper`)}>
              Top Open Skippers
            </h2>
            <table className='raceByRaceTable'>
              <tbody>
                {topOpenSkippers.slice(0, 10).map((sailor, index) => (
                  <tr className='clickable' onClick={() => nav(`/rankings/${sailor.key}`)}>
                    <td>{index + 1}</td>
                    <td className='tableColFit'>
                      <img style={{ display: 'inline', maxHeight: '3rem', minHeight: 10, minWidth: '3rem' }} src={`https://scores.collegesailing.org/inc/img/schools/${teamCodes[sailor.team]}.png`} />
                    </td>
                    <td>{sailor.name}</td>
                    <td>{sailor.rating.toFixed(0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {/* <div className='flexGrowChild contentBox'>
            <h2 onClick={() => nav(`/rankings/skipper/women`)} className='clickable'>
              Top Women's Skippers
            </h2>

            <table className='raceByRaceTable'>
              <tbody>
                {topWomenSkippers.slice(0, 10).map((sailor, index) => (
                  <tr className='clickable' onClick={() => nav(`/rankings/${sailor.key}`)}>
                    <td>{index + 1}</td>
                    <td className='tableColFit'>
                      <img style={{ display: 'inline', maxHeight: '3rem', minHeight: 10, minWidth: '3rem' }} src={`https://scores.collegesailing.org/inc/img/schools/${teamCodes[sailor.team]}.png`} />
                    ...
                    <td>{sailor.name}</td>
                    <td>
                      {sailor.rating.toFixed(0)} <FaDiamond className='secondaryText' />
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className='flexRowContainer' style={{ width: '100%' }}>
          <Link className='flexGrowChild' to={'/rankings/skipper'}>
            <button className='buttonBG' style={{ textAlign: 'center' }}>
              Top Open Skippers
            </button>
          </Link>
          <Link className='flexGrowChild' to={'/rankings/crew'}>
            <button className='buttonBG'>Top Open Crews</button>
          </Link>
          <Link className='flexGrowChild' to={'/rankings/skipper/women'}>
            <button className='buttonBG'>Top Women's Skippers</button>
          </Link>
          <Link className='flexGrowChild' to={'/rankings/crew/women'}>
        ...
        <ul>
          ...
          <li>
            <strong>Percentage:</strong> This is the percentage of the fleet that was beat in each race. If you placed last, you get 0%. If you place first, you should get 100%.
          </li>
          <li>
