...
import { getDoc, getFirestore, doc, writeBatch, collection, updateDoc, arrayUnion } from 'firebase/firestore'
...
function TeamNameForm() {
  ...
  const onSubmit = async (e) => {
    ...
    batch.set(teamDoc, { teamName: teamnameValue, owner: user?.uid, members: [{ uid: user?.uid, username: userVals?.username, displayName: userVals?.displayName, role: 'Owner' }], requests: [] })
    batch.set(teamnameDoc, { id: teamDoc.id })

    await batch.commit()
    let d = doc(db, `users/${user?.uid}`)
    await updateDoc(d, { teams: arrayUnion(teamnameValue) })
    navigate(`/teams/${teamnameValue}`)
  }
